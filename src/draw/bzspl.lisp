    ...
    (labels
      ((-to-res (pts) (veq:f$_ (map 'list #'second pts)))
       (-resappend (res a (:va 2 av))
         (declare (vector res))
         (if (or (< (length res) 1)
                 (> a (car (auxin:vector-last res))))
           (auxin:vextend (list a (list av)) res)))
       (-midsample (l r)
         (declare (veq:ff l r))
...
(veq:fvdef pos* (b xx)
  ...
  (with-struct (bzspl- ns vpts) b
    ...
    (loop with res of-type veq:fvec = (veq:f$make :dim 2 :n (length xx))
          ...
          do (setf (veq:2$ res i) (-x-to-pt vpts ns x))
          finally (return res))))
...
(defmacro -set (vpts opts a b)
  `(veq:fvprogn (setf (veq:2$ ,vpts ,b) (veq:f2$ ,opts ,a))))

(defmacro -set-mean (vpts opts a b c)
  `(veq:fvprogn (setf (veq:2$ ,vpts ,c)
